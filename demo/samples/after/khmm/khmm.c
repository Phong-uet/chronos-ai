...
hmm_par_t *hmm_new_par(int m, int n)
{
	...
	if (hp->a0 == 0) { free(hp); return 0; }
	hp->a = (FLOAT**)calloc2(n, n, sizeof(FLOAT));
	if (hp->a == 0) { free(hp->a0); free(hp); return 0; }
	hp->e = (FLOAT**)calloc2(m + 1, n, sizeof(FLOAT));
	if (hp->e == 0) {
		int j; for (j = 0; j != n; ++j) free(hp->a[j]);
		free(hp->a); free(hp->a0); free(hp); return 0;
	}
	hp->ae = (FLOAT**)calloc2((m + 1) * n, n, sizeof(FLOAT));
	if (hp->ae == 0) {
		int j;
		for (j = 0; j != n; ++j) free(hp->a[j]);
		for (j = 0; j <= m; ++j) free(hp->e[j]);
		free(hp->a); free(hp->e); free(hp->a0); free(hp); return 0;
	}
...
hmm_exp_t *hmm_new_exp(const hmm_par_t *hp)
{
	...
	if (he->A0 == 0) { free(he); return 0; }
	he->A = (FLOAT**)calloc2(hp->n, hp->n, sizeof(FLOAT));
	if (he->A == 0) { free(he->A0); free(he); return 0; }
	he->E = (FLOAT**)calloc2(hp->m + 1, hp->n, sizeof(FLOAT));
	if (he->E == 0) {
		for (i = 0; i != hp->n; ++i) free(he->A[i]);
		free(he->A); free(he->A0); free(he); return 0;
	}
	return he;
}
...
FLOAT hmm_Viterbi(const hmm_par_t *hp, hmm_data_t *hd)
{
	...
	if (preV == 0) {
		...
		free(la); free(le); free(Vmax); free(hd->v); hd->v = 0; return -HMM_INF;
	}
	...
	if (curV == 0) {
		...
		for (b = 0; b <= hp->m; ++b) free(le[b]);
		for (u = 0; u <= hd->L; ++u) free(Vmax[u]);
		free(la); free(le); free(Vmax); free(preV); free(hd->v); hd->v = 0; return -HMM_INF;
	}
	for (k = 0; k != hp->n; ++k)
		for (l = 0; l != hp->n; ++l)
...
void hmm_forward(const hmm_par_t *hp, hmm_data_t *hd)
{
	...
	if (at == 0) {
		for (k = 0; k <= hd->L; ++k) free(hd->f[k]);
		free(hd->f); hd->f = 0; free(hd->s); hd->s = 0; return;
	}
	// transpose a[][]
	for (k = 0; k != n; ++k)
		for (l = 0; l != n; ++l)
