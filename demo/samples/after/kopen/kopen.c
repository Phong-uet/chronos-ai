...
static int http_open(const char *fn)
{
	...
	if (host == NULL || port == NULL || path == NULL) {
		free(http_host); free(host); free(port); free(path);
		return -1;
	}
	...
	while (read(fd, buf + l, 1)) { // read HTTP header; FIXME: bad efficiency
		if (buf[l] == '\n' && l >= 3)
			if (strncmp(buf + l - 3, "\r\n\r\n", 4) == 0) break;
		++l;
		if (l >= 0xffff) break; // prevent buf overflow: stop one byte before end
	}
	buf[l] = 0;
...
static int kftp_get_response(ftpaux_t *aux)
{
	...
	while (read(aux->ctrl_fd, &c, 1)) { // FIXME: this is *VERY BAD* for unbuffered I/O
		if (n >= aux->max_response) {
			int new_max = aux->max_response? aux->max_response<<1 : 256;
			char *new_response = realloc(aux->response, new_max);
			if (new_response == NULL) return -1;
			aux->response = new_response;
			aux->max_response = new_max;
		}
		aux->response[n++] = c;
		if (c == '\n') {
...
static int ftp_open(const char *fn)
{
	...
	host = calloc(l + 1, 1);
	if (host == NULL) { free(port); return -1; }
	strncpy(host, fn + 6, l);
	retr = calloc(strlen(p) + 8, 1);
	if (retr == NULL) { free(host); free(port); return -1; }
	sprintf(retr, "RETR %s\r\n", p);
...
static char **cmd2argv(const char *cmd)
{
	...
	if (str == NULL) { free(argv); return 0; }
	...
	if (strstr(fn, "http://") == fn) {
		...
		if (*p == '<') { // pipe open
			...
			if (pid == 0) { /* the child process */
				...
				close(pfd[1]);
				aux = calloc(1, sizeof(koaux_t));
				if (aux == NULL) { close(pfd[0]); return 0; }
				aux->type = KO_PIPE;
				aux->fd = pfd[0];
				aux->pid = pid;
			}
			...
			*_fd = open(fn, O_RDONLY);
			...
			if (*_fd) {
				aux = calloc(1, sizeof(koaux_t));
				if (aux == NULL) { close(*_fd); *_fd = -1; return 0; }
				aux->type = KO_FILE;
				aux->fd = *_fd;
			}
		}
