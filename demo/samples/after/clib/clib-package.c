    ...
    if (flags) {
      for (unsigned int i = 0; i < json_array_get_count(flags); i++) {
        ...
        if (flag) {
          ...
          if (-1 == asprintf(&pkg->flags, "%s %s",
                             old_flags ? old_flags : "", flag)) {
            free(old_flags);
            free(flag);
            goto cleanup;
          }
...
#endif
      // FIX: check res for NULL before dereferencing; only read data when ok
      if (!res || !res->ok) {
        goto download;
      }
      json = res->data;
...
static int fetch_package_file_work(clib_package_t *pkg, const char *dir,
  ...
  // FIX: make a local copy before passing to basename(), which may modify
  // its argument on some platforms, and strdup the result before freeing
  // the copy so that base_path is always heap-allocated.
  ...
  char *base_path = strdup(basename(file_copy));
  free(file_copy);
...
int clib_package_install_executable(clib_package_t *pkg, const char *dir,
  ...
  if (pkg->flags) {
    ...
    // FIX: use the newly built `flags` string (not cflags) and free it after
    if (flags) {
