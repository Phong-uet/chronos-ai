...
int configure_package_with_manifest_name(const char *dir, const char *file) {
  ...
  if (0 != package->dependencies) {
    ...
    while ((node = list_iterator_next(iterator))) {
      ...
      // FIX: check asprintf result before using slug
      if (-1 == asprintf(&slug, "%s/%s@%s", dep->author, dep->name, dep->version)) {
        rc = -ENOMEM;
      ...
      // FIX: guard against NULL dependency before dereferencing ->name
      if (0 == dependency) {
        rc = -ENOMEM;
        list_iterator_destroy(iterator);
        goto cleanup;
